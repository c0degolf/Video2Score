...
2. Open `run.py` to configure the following settings:
   - `VIDEO`: Set the file path to your target video (e.g., `"./source/V.mp4"`).
   - `BPM`: Adjust the tempo in the `create_midi.generate_midi()` function to match your video's performance speed.
   - `TIME_SIGNATURE`: Adjust the Time Signature in the `create_midi.generate_midi()` function to match your video's meter (e.g., `"4/4"`).
...
- You can adjust the parameter in `run.py` to match the performance tempo.
- Output file: `output.mid`


## Experimental Features

The following parameters are currently under development and may change in future versions.
They can be adjusted in `run.py` to improve MIDI timing depending on the input video.

- `MISS_GRID`
- `REST_IGNORE_FRAMES`
- `ONSET_MERGE_TOLERANCE`
